...
final transactionByCategoryProvider = Provider.family<
  AsyncValue<({List<TransactionModel> transactions, double total})>,
  String
>((ref, categoryId) {
  final transactionAsync = ref.watch(filteredTransactionByDateRangeProvider);
...
final totalAmountByCategoryProvider = Provider.family<double, String>((
  ref,
  categoryId,
) {
  final transactionAsync = ref.watch(filteredTransactionByDateRangeProvider);
...
final enrichedTransactionProvider = Provider<
  AsyncValue<List<({TransactionModel transaction, Category? category})>>
>((ref) {
  final transactionAsync = ref.watch(filteredTransactionByDateRangeProvider);
          ...
          return transactions
              .map(
                (transactionData) => (
                  ...
                  category: categoryMap[transactionData.categoryId],
                ),
              )
              .toList();
        })
...
final enrichedTransactionByTypeProvider = Provider.family<
  AsyncValue<List<({TransactionModel transaction, Category? category})>>,
  TransactionType
>((ref, type) {
  final enrichedTransactionAsync = ref.watch(enrichedTransactionProvider);
  return enrichedTransactionAsync.whenData(
    (value) =>
        value.where((element) => element.transaction.type == type).toList(),
  );
});
...
>((ref, type) {
  ...
  return categoryAsync.whenData((enrichedList) {
    final grouped = CollectionUtils.groupBy(
      enrichedList,
        ...
        grouped.entries.map((entry) {
          ...
          final total = items.fold(
            ...
            (previousValue, element) =>
                previousValue + element.transaction.amount,
          );
          final percentage =
...
final groupPieChartProvider =
    Provider.family<AsyncValue<Map<String, double>>, TransactionType>((
      ref,
      type,
    ) {
      ...
      return groupAsync.whenData((groups) {
        return {
          for (var group in groups) group.category?.name ?? 'Khác': group.total,
        };
      });
    });

/// Nhóm giao dịch được làm giàu theo ngày để hiển thị có tổ chức trong danh sách giao diện
final enrichedTransactionGroupByDateProvider = Provider<
  AsyncValue<
    Map<DateTime, List<({TransactionModel transaction, Category? category})>>
  >
>((ref) {
  final enrichedTransactionAsync = ref.watch(enrichedTransactionProvider);
  ...
  return enrichedTransactionAsync.whenData((enrichedList) {
    return CollectionUtils.groupByAndSort(
      enrichedList,
      (item) => DateRangeUtils.dateOnly(item.transaction.date),
      descending: true,
    );
  });
});
