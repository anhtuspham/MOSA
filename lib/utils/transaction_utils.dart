...
class TransactionAggregator {
  static ({double income, double expense}) calculatorTotals(List<TransactionModel> transactions) {
    ...
    for (var transaction in transactions) {
      final balanceEffect = TransactionTypeManager.getBalanceEffect(transaction.type);
  ...
  static double sumByBalanceEffect(List<TransactionModel> transactions, BalanceEffect effect) {
    return transactions
        .where((t) => TransactionTypeManager.getBalanceEffect(t.type) == effect)
        .fold(0.0, (sum, t) => sum + t.amount);
  }

  static double sumByCategory(List<TransactionModel> transactions, TransactionCategory category) {
    return transactions
  ...
  static double calculateBalanceImpact(List<TransactionModel> transactions) {
    return transactions.fold(0.0, (balance, transaction) {
      return balance + TransactionTypeManager.calculateBalanceImpact(
        transaction.type, 
        transaction.amount
      );
    });
  ...
  static List<TransactionModel> filterByCategory(List<TransactionModel> transactions, TransactionCategory category) {
    return transactions
        .where((t) => TransactionTypeManager.getCategory(t.type) == category)
        .toList();
  }

  static List<TransactionModel> filterByBalanceEffect(List<TransactionModel> transactions, BalanceEffect effect) {
    return transactions
        .where((t) => TransactionTypeManager.getBalanceEffect(t.type) == effect)
        .toList();
  }
